...
<div class="max-w-2xl mx-auto px-4 py-8 font-sans tracking-wide">
    <div class="mb-8 border-b border-zinc-100 pb-4">
        <h1 class="text-2xl font-light text-zinc-900 uppercase tracking-widest">Criar Categoria</h1>
        <p class="text-xs text-zinc-500 mt-1">Adicionar uma nova secção ao catálogo público</p>
    </div>
    ...
    <form action="{{ route('admin.categories.store') }}" method="POST" enctype="multipart/form-data" class="space-y-6">
        ...
        <div>
            ...
            <input type="text" 
                   ...
                   placeholder="Ex: Frases Engraçadas, Geek, Desporto..." 
                   required>
            @error('name')
        ...
        <div>
            <label class="block text-xs uppercase tracking-widest text-zinc-500 font-medium mb-2">Imagem de Capa (Opcional)</label>
            <div class="border border-dashed border-zinc-200 bg-zinc-50/50 p-6 rounded-sm text-center">
                ...
                <p class="text-xxs text-zinc-400 mt-2">Formatos aceites: JPG, PNG, GIF (Máx. 2MB)</p>
            </div>
            @error('file_image')
                <p class="text-xs text-red-500 mt-1 font-light">{{ $message }}</p>
            @enderror
