<x-app-layout>
    ...
    <div class="flex space-x-3">
        <a href="{{ route('customer.orders.index') }}"
            class="text-sm bg-white hover:bg-gray-200 text-gray-700 font-bold py-2 px-4 rounded transition shadow-sm">
            ← Voltar ao Histórico
        </a>
    </div>
            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg p-6">
                ...
                <div class="border-b border-gray-200 pb-4 mb-6 sm:flex sm:justify-between sm:items-center">
                    <div>
                        ...
                        <p class="text-sm text-gray-600 mt-1">Método de Pagamento: <span
                                class="font-semibold text-gray-900 uppercase">
                ...
                <div class="mb-8">
                    <h3 class="text-md font-bold text-gray-900 mb-4 uppercase tracking-wider text-xs text-gray-400">
                        Artigos Adquiridos</h3>
                            ...
                            <div class="py-4 flex justify-between items-center">
                                <div>
                                    <h4 class="font-bold text-gray-900 text-sm">
                                        {{ $item->tshirtImage->name ?? 'Estampa Personalizada' }}</h4>
                ...
                <div
                    class="grid grid-cols-1 md:grid-cols-2 gap-6 bg-gray-50/70 p-4 rounded-lg border border-gray-100 mb-6">
                    <div>
                        <h4 class="text-xs font-bold uppercase tracking-wider text-gray-400 mb-2">Dados de Envio</h4>
                        <p class="text-sm text-gray-800 leading-relaxed"><span class="text-gray-400">Morada:</span>
                            {{ $order->address }}</p>
                        @if(!empty($order->notes))
                            <p class="text-sm text-gray-800 mt-2 leading-relaxed"><span class="text-gray-400">Notas do
                                    Cliente:</span> {{ $order->notes }}</p>
                        @endif
                    </div>
                    <div>
                        <h4 class="text-xs font-bold uppercase tracking-wider text-gray-400 mb-2">Dados de Faturação
                        </h4>
                        <p class="text-sm text-gray-800"><span class="text-gray-400">NIF:</span>
                            {{ $order->nif ?? 'Consumidor Final' }}</p>
                        <p class="text-sm text-gray-800 mt-1"><span class="text-gray-400">Ref. Transação:</span>
                            {{ $order->payment_ref }}</p>
                    </div>
                ...
                <div class="flex justify-between items-center pt-4 border-t border-gray-200 font-bold text-gray-900">
                    ...
                    <span
                        class="text-2xl font-black text-indigo-600">{{ number_format($order->total_price, 2, ',', ' ') }}
                        €</span>
                </div>
